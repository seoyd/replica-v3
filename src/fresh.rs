...
pub enum Command {
    ...
    PairedPrepare {
        ...
        #[arg(long, conflicts_with_all = ["learning_rate_threefold", "co_batch"])] first_target_four: bool,
        /// Bounded LR-only trial: same P6144/data/Adam, coefficient1, constant9e-5.
        #[arg(long, conflicts_with = "co_batch")] learning_rate_threefold: bool,
        /// Same batch8 and two-update multiset, with selector pairs in one batch.
        #[arg(long)] co_batch: bool,
    },
...
pub fn execute(command: Command) -> Result<()> {
    match command {
        ...
        Command::PairedPrepare { parent, source_data, output, parity, first_target_four, learning_rate_threefold, co_batch } => paired_prepare(&parent, &source_data, &output, parity.as_deref(), first_target_four, learning_rate_threefold, co_batch),
...
fn plan_read_bound(root: &Path, source: &str, executable: &str) -> Result<Plan> {
    ...
    let expected = if let Some(f) = &p.fork {
        ...
        if file_hash(&f.study.join("study.r3b"))? != f.study_hash
            ...
            || ([7,8].contains(&study.schema) && study.updates!=if p.tiny {2}else{1280})
        ...
        if let Some(tape) = &p.paired {
            ...
            if ![4,5,6,7,8].contains(&study.schema) || !["SPACED", "ADJACENT", "COBATCH"].contains(&tape.mode.as_str())
                ...
                || tape.rows.iter().any(|r| r.iter().enumerate().any(|(b, i)| *i >= 4*n || (study.schema!=8 && !p.order[b].contains(&(i%n)))))
        ...
        } else if [4,5,6,7,8].contains(&study.schema) { return Err(bad("paired study missing finite tape")); }
        for (name, h) in [
...
fn paired_decision(p: &Plan, root: &Path, step: usize) -> Result<binary::Value> {
    ...
    if full {
        ...
        let study:Study=read(&f.study.join("study.r3b"))?;
        if [5,6,7,8].contains(&study.schema) && !joint && stop.is_none() && !p.tiny {
...
fn verify_pair_tape(parent: &Plan, tm: &[Meta], tape: &PairTape) -> Result<()> {
    let arm = match tape.mode.as_str() { "SPACED" => 0, "ADJACENT" | "COBATCH" => 1, _ => return Err(bad("unknown pair policy")) };
    let mut expected = pair_tapes(parent, tm, tape.block, tape.rows.len())?;
    if tape.mode=="COBATCH" { expected[1]=co_batch_rows(&expected[1])?; }
...
fn paired_prepare(parent: &Path, source_data: &Path, output: &Path, parity: Option<&Path>, first_target_four: bool, learning_rate_threefold: bool, co_batch: bool) -> Result<()> {
    if [first_target_four,learning_rate_threefold,co_batch].into_iter().filter(|x|*x).count()>1 {return Err(bad("only one research intervention may be registered"));}
    let single = first_target_four || learning_rate_threefold || co_batch;
    ...
    let updates=if first_target_four {tapes[1].len()}else if learning_rate_threefold || co_batch {if pp.tiny {2}else{1280}}else{block};
    ...
    if single {
        ...
        write(&path,&binary::record!({"scope":if co_batch {"USER_AUTHORIZED_SINGLE_VARIABLE_COBATCH_20260920"}else if learning_rate_threefold {"USER_AUTHORIZED_SINGLE_VARIABLE_LR9E5_20260920"}else{"USER_AUTHORIZED_SINGLE_VARIABLE_FIRST_TARGET4_20260920"},
            "hypothesis":if co_batch {"joint selector-pair gradients at unchanged batch8, LR, loss, Adam and two-update case multiset"}else if learning_rate_threefold {"test adaptation rate with fixed threefold LR; retained coefficient-1 LR3e-5 control at matched cursors"}else{"increase first-response-token learning weight; retained coefficient-1 control at matched tape cursors"},
            "first_target_weight":if first_target_four {4.}else{1.},"constant_lr":lr,
            "objective":if first_target_four {"(sum response NLL + 3 * sum first-target NLL) / actual response token count"}else{"sum response NLL / actual response token count"},
            ...
            "control_absolute_steps":if pp.tiny {vec![state.step+updates]}else if learning_rate_threefold || co_batch {vec![6400,7424]}else{vec![6400,7424,8448,9984]},"prior_candidate_promoted":false}))?;
        inventory.push((PathBuf::from("research-authorization.r3b"),file_hash(&path)?,std::fs::metadata(&path)?.len()));
    }
    let study=Study {schema:if co_batch {8}else if learning_rate_threefold {7}else if first_target_four {6}else{4},source:source_digest()?,binary:file_hash(&std::env::current_exe()?)?,parent:parent.clone(),
        ...
        tie_break:if co_batch {vec!["COBATCH".into()]}else if single {vec!["ADJACENT".into()]}else{vec!["SPACED".into(),"ADJACENT".into()]}};
    write(&output.join("study.r3b"),&study)?;
    for arm in &study.tie_break {
        let tape=if arm=="COBATCH" {co_batch_rows(&tapes[1])?}else{tapes[usize::from(arm=="ADJACENT")].clone()};
...
fn paired_work(root:&Path)->Result<(u64,u64,u64)> {
    ...
    if used.0>12_000_000||used.1>1_000_000||used.2>if [5,6,7,8].contains(&study.schema) {study.updates as u64}else{4096}{return Err(bad("paired global learning budget"));}
    Ok(used)
}
fn paired_usage(root:&Path)->Result<(f64,usize,usize)> {
    let s=study_read(root)?;if ![4,5,6,7,8].contains(&s.schema){return Err(bad("paired study schema"));}
    ...
    if !s.tiny && [4,6,7,8].contains(&s.schema) {
        generation+=16;
...
fn paired_report(root:&Path)->Result<binary::Value> {
    ...
    Ok(binary::record!({"source":s.source,"binary":s.binary,"results":results,"same_budget":endpoints,
        "paired_gain_loss":comparisons,"both_gain_loss":both_gain_loss,"effect":if s.schema==8 {"COBATCH_RETAINED_CONTROL_COMPARISON_SEPARATE"}else if s.schema==7 {"LR9E5_RETAINED_CONTROL_COMPARISON_SEPARATE"}else if s.schema==6 {"FIRST_TARGET4_RETAINED_CONTROL_COMPARISON_SEPARATE"}else if s.schema==5 {"SINGLE_ARM_EXPOSURE_NOT_COMPARISON"}else if endpoints {"EQUAL_BUDGET"}else if endpoint_steps[0]==endpoint_steps[1]{"EARLY_STOPS_NO_256_COMPARISON"}else{"UNEQUAL_BUDGET_NOT_ESTABLISHED"},
        "selected_for_continuation":eligible.first().map(|v|&v.0),"usage":usage,"training_usage":paired_work(root)?,
        "result":if s.schema==8 {"COBATCH_CLOSED_CHECK_JOINT_RESULT"}else if s.schema==7 {"LR9E5_CLOSED_CHECK_JOINT_RESULT"}else if s.schema==6 {"FIRST_TARGET4_CLOSED_CHECK_JOINT_RESULT"}else if s.schema==5 {"FOLLOW_THROUGH_CLOSED_CHECK_JOINT_RESULT"}else if eligible.is_empty(){"STUDY_COMPLETE_NO_ADMISSIBLE_LEARNER"}else{"G3_ENTRY_PASS"},"GOAL1_READY":false}))
}
...
fn study_read_bound(root: &Path, frozen_executable: Option<&Path>) -> Result<Study> {
    ...
    if ![2, 3, 4, 5, 6, 7, 8].contains(&s.schema)
...
mod tests {
    ...
    fn paired_tape_full_writer_reader_bounds_and_balance() {
        ...
        for (mode, rows) in [("SPACED",&tapes[0]),("ADJACENT",&tapes[1]),("COBATCH",&co)] {
            ...
            for block in rows.chunks_exact(256) {
                for bucket in 2..=4 {
                    ...
                    for entries in pairs.values() {
                        ...
                        assert_eq!(entries[1].0-entries[0].0,if mode=="SPACED" {128}else if mode=="COBATCH" {0}else{1});
