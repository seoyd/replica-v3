...
이어진 가중치 고정 진단은 QA0/QA2 원문35/136, 질문 치환64/136, 근거 하나86/136,
둘을 적용115/136이었다. oracle 점수는 품질 판정에서 제외한다. 모델의 근거 선택/질문 전이와
식별자 복사 실패를 구분한 관측이며, 추가 학습이나 S5/S6를 자동 시작하지 않는다.
...
실패 실행을 자동 연장하지 않는다. 위3700updates는 V 이전 합계이며, 이번 V1000은 별도다.
같은 원본 QA 개발 기준 현재 후보는 V1000이다. 최종 시험 적격이나 운영 채택은 아니다.
